...
HRESULT CCastle::Ready_GameObject(void * pArg)
{
	...
	if (FAILED(pManagement->Add_Object_ToLayer(SCENE_STAGE, L"GameObject_Gate", SCENE_STAGE, L"Layer_Unit_Enemy",this)))
		return E_FAIL;
...
HRESULT CCastle::Add_Component()
{
	...
	// For.Com_Shader
	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Shader_Castle", L"Com_Shader", (CComponent**)&m_pShaderCom)))
		return E_FAIL;

	// For.Com_Texture

	if (FAILED(CGameObject::Add_Component(SCENE_STAGE, L"Component_Texture_Castle", L"Com_Texture", (CComponent**)&m_pTexture)))
		return E_FAIL;
...
HRESULT CCastle::SetUp_ConstantTable()
{
	...
	m_pShaderCom->Set_Value("g_vLightAmbient", &_float4(1.f,0.9f,0.9f,1.f), sizeof(_float4));
