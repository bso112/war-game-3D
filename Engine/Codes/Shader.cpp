...
HRESULT CShader::Set_Matrix(D3DXHANDLE hParameter, const _matrix* _matrix)
{
	if (nullptr == m_pEffect)
		return E_FAIL;

	return m_pEffect->SetMatrix(hParameter, _matrix);
}

HRESULT CShader::Set_Bool(D3DXHANDLE hParameter, bool _bool)
{
	if (nullptr == m_pEffect)
		return E_FAIL;

	return m_pEffect->SetBool(hParameter, _bool);
}
