...
CGameObject* CObject_Manager::Add_Object_ToLayer(_uint iPrototypeSceneID, const _tchar * pPrototypeTag, _uint iLayerSceneID, const _tchar * pLayerTag, void* pArg)
{
	if (m_iNumScenes <= iPrototypeSceneID ||
		m_iNumScenes <= iLayerSceneID)
		return nullptr;
	...
	CLayer* pLayer = Find_Layer(iLayerSceneID, pLayerTag);
	if (nullptr == pLayer)
	{
		...
		m_pLayers[iLayerSceneID].insert(LAYERS::value_type(pLayerTag, pLayer));
	}
	else
	{
		if (FAILED(pLayer->Add_Object(pGameObject)))
...
_int CObject_Manager::Update_Object_Manager(_double TimeDelta)
{
	for (size_t i = 0; i < m_iNumScenes; ++i)
	{
		for (auto& Pair : m_pLayers[i])
		{
			...
			{
				if (1 == Pair.second->Update_Layer(TimeDelta))
					return 1;
...
void CObject_Manager::Free()
{
	...
	{
		for (auto& Pair : m_pLayers[i])
			Safe_Release(Pair.second);	// Áß´ÜÁ¡
